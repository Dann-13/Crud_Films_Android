package com.example.aplicacionpelis.datos;
...
public class PeliculasBD extends SQLiteOpenHelper implements RepositorioPeliculas{
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE peliculas (_id INTEGER PRIMARY KEY AUTOINCREMENT, nombrePelicula TEXT, director TEXT, sinopsis TEXT, " +
                "paisOrigen TEXT, genero INTEGER,anioLanzamiento INTEGER) ");
    }
    @Override
    ...
    public int nuevo() {
        ...
        getWritableDatabase().execSQL("INSERT INTO peliculas (nombrePelicula, director, sinopsis, paisOrigen, genero, anioLanzamiento) " +
        " VALUES('','','','', "+pelicula.getTipoGenero().ordinal() +",'' ) ");
        Cursor c = getReadableDatabase().rawQuery("SELECT _id FROM peliculas WHERE genero = " + pelicula.getTipoGenero().ordinal(), null);
        if (c.moveToNext()) _id = c.getInt(0);
        c.close();
        Log.d("TAG","nueva peli DB "+_id+" "+pelicula.getNombrePelicula());
        return _id;
    }
    @Override
    ...
    public void actualiza(int id, Pelicula movie) {
        getWritableDatabase().execSQL("UPDATE peliculas SET" +
                " nombrePelicula = '"+ movie.getNombrePelicula() +"', " +
                " director = '" + movie.getDirector() + "'," +
                " sinopsis = '" + movie.getSinopsis() + "'," +
                " paisOrigen = '" + movie.getPaisOrigen() + "'," +
                " genero = " + movie.getTipoGenero().ordinal() +
                ", anioLanzamiento = " +movie.getAnioLanzamiento()+
                " WHERE _id = " + id);
    }
